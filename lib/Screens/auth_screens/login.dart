...
class LoginScreen extends StatefulWidget {
  LoginScreen({Key? key, this.title, required this.isaccountapprovalbyadminneeded, required this.accountApprovalMessage, required this.prefs, required this.doc, required this.isblocknewlogins}) : super(key: key);
    ...
    try {
      ...
      if (firebaseUser != null) {
        ...
        if (documents.length == 0) {
          Fiberchat.toast("User not exists.");
        } else {
          ...
          if (fcmToken != null) {
            ...
            await widget.prefs.setString(Dbkeys.nickname, documents[0][Dbkeys.nickname]);
      ...
      print('Error occured while verifying phone number: $e');
      setState(() {
              ...
              children: <Widget>[
                ...
                Padding(
                  ...
                  child: Text(
                    ...
                    //           spacing: 0.3,
                    //           height: 57,
                    // buttoncolor: storychatGrey.withOpacity(0.5),
          ...
          isShowCompletedLoading == true
              ...
              : Consumer<TimerProvider>(
                  builder: (context, timeProvider, _) {
                    return timeProvider.wait == true && isCodeSent == true
                        ? Padding(
                              ...
                              await FirebaseFirestore.instance.collection(DbPaths.collectionusers).get().then((value) {
                                value.docs.forEach((element) {
                                  if (element[Dbkeys.username] == _username.text) {
                                    Map<String, String> result = extractPhoneNumberAndCountryCode(List<String>.from(element[Dbkeys.phonenumbervariants]));
                                    String phoneNumber = result["phoneNumber"]!;
                                    String countryCode = result["countryCode"]!;

                                    if (phoneCode == countryCode && phoneNumber == _phoneNo.text) {
                                      setState(() {
                                        _usernameValid = true;
                                      });
                                    }
...
class InpuTextBox extends StatefulWidget {
  ...
  InpuTextBox({this.controller, required this.isDark, this.boxbordercolor, this.boxheight, this.fontsize, this.leftrightmargin, this.topbottommargin, this.letterspacing, this.forcedmargin, this.boxwidth, this.boxcornerradius, this.boxbcgcolor, this.hinttext, this.boxborderwidth, this.onSaved, this.textCapitalization, this.onchanged, this.placeholder, this.showIconboundary, this.subtitle, this.disabled, this.keyboardtype,
      this.inputFormatter,
      this.validator,
      this.title,
      this.maxLines,
